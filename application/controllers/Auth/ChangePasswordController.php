...
class ChangePasswordController extends CI_Controller {
	...
	public function index()
	{
		...
		if ($this->form_validation->run() == FALSE) :
			...
			$this->load->view('dashboard/user/ganti_password');
	...
	public function change_password($passwordSekarang, $passwordBaru)
	{
		...
		if ($mahasiswa == TRUE) :
			...
			if (password_verify($passwordSekarang, $mahasiswa['password'])) :

				$params = [
					'password' => password_hash($passwordBaru, PASSWORD_DEFAULT),
				];

				$resp = $this->db->update('mahasiswa',$params,['nim' => $mahasiswa['nim'] ]);
				if ($resp) :
